...
##
    ...
    #lights on and off every eight hours
    #Light levels during night
    if (((sensorsG.time - day_clock < 28800) or (sensorsG.time - day_clock > 57600))):
       if(lights_on == True):
           print("dim lights %1.f " % (sensorsG.time - day_clock))
           led_pub.publish(240)
           lights_on = False

    #Light levels during day
    elif ((sensorsG.time - day_clock > 28800) and (sensorsG.time - day_clock < 57600)):
       if(lights_on == False):
           print("increase brightness %.1f" % (sensorsG.time - day_clock))
           led_pub.publish(210)
           lights_on = True

    #Fans (on and off every 15 mins)
    if (((sensorsG.time - day_clock) % 300) > 0 and ((sensorsG.time - day_clock) % 300) < 300):
       if((fans_on == False) and (sensorsG.temperature>23.5)):
           print("fans on at %.1f" % (sensorsG.time - day_clock))
           fan_pub.publish(True)
           fans_on = True
    elif(((sensorsG.time - day_clock) % 300) > 300):
       if(fans_on == True):
           print("fan off at %.1f" % (sensorsG.time - day_clock))
